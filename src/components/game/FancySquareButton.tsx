...
interface FancySquareButtonProps {
  onClick: () => void;
  cutoutImage: string;
  title?: string;
  className?: string;
...
export function FancySquareButton({
  onClick,
  cutoutImage,
  title,
  className = '',
}: FancySquareButtonProps) {
  return (
    <button
      onClick={onClick}
      title={title}
      className={`fancy-square-button group relative cursor-pointer ${className}`}
      style={{
        width: '36px',
        height: '36px',
      }}
    >
      ...
      <div
        ...
        style={{
          backgroundImage: 'url(/BlankSquareButton.png)',
          backgroundSize: '100% 100%',
          backgroundPosition: 'center',
        }}
      />

      {/* Glowing symbol layer - masked to only show where symbol is */}
      <div
